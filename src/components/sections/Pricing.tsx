...
export function Pricing() {
  ...
  return (
    <section id="pricing" className="py-24 bg-bg scroll-mt-16">
      <div className="container mx-auto px-4">
        ...
        <motion.p
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-text-secondary text-lg text-center mb-16 max-w-2xl mx-auto"
        >
          {t('subtitle')}
        </motion.p>
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl p-6 border shadow-lg cursor-pointer transition-all duration-200 ${
                  selectedPlan === plan
                    ? 'border-primary bg-bg shadow-primary/10'
                    : plan === 'yearly' && !selectedPlan
                      ? 'border-primary bg-bg relative shadow-primary/10'
                      : 'border-bg-surface bg-bg-secondary'
                }`}
              >
                {selectedPlan === plan && (
                  <span className="absolute -top-3 right-4 bg-primary text-white text-xs font-medium px-3 py-1 rounded-full flex items-center gap-1">
                ...
                <div className="text-sm text-text-secondary mb-4">{p.unit}</div>
                <div className="text-xs text-text-tertiary mb-6">{p.daily}</div>
        ...
        >
          ...
          <div className="bg-bg-secondary rounded-2xl border border-bg-surface overflow-x-auto">
            <div className="min-w-[600px] grid grid-cols-5 gap-0">
              <div className="p-3 text-sm font-medium text-text-secondary border-b border-bg-surface" />
              <div className="p-3 text-sm font-medium text-text-secondary text-center border-b border-bg-surface">
                {t('freePlan')}
              </div>
              <div className="p-3 text-sm font-medium text-text-light text-center border-b border-bg-surface">
                {t.raw('plans.monthly.name') as string}
              </div>
              <div className="p-3 text-sm font-medium text-text-light text-center border-b border-bg-surface">
                {t.raw('plans.quarterly.name') as string}
              </div>
              <div className="p-3 text-sm font-medium text-primary-light text-center border-b border-bg-surface">
                {t.raw('plans.yearly.name') as string}
              </div>
              {Object.entries(comparison).map(([key, val]) => (
                <div key={key} className="contents">
                  <div className="p-3 text-sm text-white border-b border-bg-surface">
                    {benefits[key]}
                  </div>
                  <div className="p-3 text-sm text-text-secondary text-center border-b border-bg-surface">
                    {val.free}
                  </div>
                  <div className={`p-3 text-sm text-center border-b border-bg-surface ${key === 'ads' || key === 'support' ? 'text-success' : 'text-text-secondary'}`}>
                    {val.monthly}
                  </div>
                  <div className={`p-3 text-sm text-center border-b border-bg-surface ${key === 'ads' || key === 'support' || key === 'routing' ? 'text-success' : 'text-text-secondary'}`}>
                    {val.quarterly}
                  </div>
                  <div className="p-3 text-sm text-success text-center border-b border-bg-surface font-medium">
                    {val.yearly}
                  </div>
                </div>
              ))}
          ...
          return (
            ...
            >
              <motion.div
                ...
                className="bg-bg-secondary rounded-2xl border border-bg-surface p-8 max-w-sm w-full relative"
              >
                <button
                ...
                <div className="text-sm text-text-secondary mb-6">{mp.unit}</div>
                ...
                <p className="text-xs text-text-tertiary text-center mt-3">{t('redirectToPayment')}</p>
              </motion.div>
            </motion.div>
          );
        })()}
